...
class GenericNode2d(iId: String = "", iPosition: Vec2 = Vec2(0.0f, 0.0f), iAnchorPoint: Vec2 = Vec2(0.5f, 0.5f)) {
    init {
        if (iId != "") {
    ...
    var children: MutableList<GenericNode2d> = mutableListOf()
        private set

    fun addChild(node: GenericNode2d) {
        require(node != this) { "A node cannot have itself as a child." }
        children.add(node)
    }

    fun getChildById(id: String): GenericNode2d {
    ...
    fun getChildAtIndex(i: Int): GenericNode2d {
    ...
    fun removeChild(node: GenericNode2d) {
